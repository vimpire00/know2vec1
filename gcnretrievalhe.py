...
from hete_loader import get_datasets_emb
from hete_qkloader import  LearnwareDataset
...
class Retrieval:
    ...
    def train(self):
        ...
        for curr_epoch in range(self.args.n_epochs):
            ...
            for step, (inputs, labels,padlength) in enumerate(self.data_loader_train):
                ...
                #torch.Size([200, 256]) torch.Size([48, 256])
            ...
            if curr_epoch%2==0:
                ...
                torch.save(model_dict,'./framwork/'+'heteframworkclass'+str(curr_epoch)+'.pth')
    ...
    def test(self):
        ...
        for step, (inputs, labels,qlength) in enumerate(self.data_loader_test):
            ...
            # print(scores.shape[0],"score_shape0")
            for i in range(scores.shape[0]):
                if i%2==0:
                    label=labels[i]
                    # print(i,test_names[i//2],"test_names[i%2]" )
                    non_zero_indices = (label != 0)
                    labell = label[non_zero_indices].detach().cpu().numpy()
                    outputt = scores[i][non_zero_indices].detach().cpu().numpy()
                    score=measure_test(outputt,labell)
                    print(score)
                    # scores_dict[test_names[i//2]]={'label':labell,'output':outputt,'score':score}
                    kscores.append(score)
            # print(scores_dict,"scores_dict")
            # torch.save(scores_dict,'./plot_dict/'+str(step)+'.pth')
    ...
    def test1(self):  #直接运行，而不是在训练的时候测试，先加载模型再测试
        ...
        self.qknet= QNET(1000, self.hidden_size, self.embsize,2).to(self.device)
        self.qknet.eval()
        model_dict=torch.load('./framwork/'+'heteframwork'+str(curr_epoch)+'.pth',map_location='cpu')
...
class Parser:
    ...
    def set_arguments(self):
        ...
        self.parser.add_argument('--embsize', type=int, default=128, help='Number of hidden units.')
